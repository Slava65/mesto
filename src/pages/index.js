...
const inputAvatarLink = document.querySelector(".popup__text_avatar");
...
//Инициализация данных страницы с сервера
const renderPage = () => {
  Promise.all([api.getInfoUser(), api.getInitialCards()])
    .then(([actualUserInfo, initialCards]) => {
      userInfo.setUserInfo(actualUserInfo);
...
//Ф-ция - параметр открывающая окно подтверждения удаления
const handleConfirmDelOpen = () => {
  confirmPopup.open();
};
...
const popupProfileObject = new PopupWithForm(".popup_profile", (data) => {
  ...
  api
    .editInfoUser({ name: data.name, about: data.about })
    .then((info) => {
...
const popupAvatar = new PopupWithForm(".popup_avatar", () => {
  ...
  const newAva = inputAvatarLink.value;
  api
    .setAva(newAva)
    .then((data) => {
      console.log(data);
      popupAvatar.close();
      avatarBtn.src = newAva;
    })
    .catch((error) => {
      console.log(error);
    })
